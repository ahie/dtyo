...
The purpose of an activation function is to alter the incoming signal to induce non-linearity into the network, enabling the learning of more complex functions. 

A common representation of a simple neural network is a graph where the output-input neuron pairs are visualized with directed edges. As an example, Figure~\ref{fig:nn} depicts a feedforward neural network\footnote{Note that bias nodes are omitted from the figure for simplicity.} taking input vectors $\mathbf{x}^{(i)}$ of length $3$ and outputting vectors $\mathbf{y}^{(i)}$ of length $2$.
