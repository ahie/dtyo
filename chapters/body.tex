...
$$\mu_k \sim \mathcal{N}(0, \sigma^2),                                      \quad k = 1, \dotsc, K,$$
$$c_i \sim \text{Categorical}(\frac{1}{K}, \dotsc, \frac{1}{K}),            \quad i = 1, \dotsc, n,$$
$$x_i \vert c_i, \mathbf{\mu} \sim \mathcal{N}(c^{\top}_i \mathbf{\mu}, 1), \quad i = 1, \dotsc, n,$$

where the latent \footnote{Note that $c_i$ is being encoded as an indicator $K$-vector, where all entries are zero except for the index that corresponds to the drawn categorical variable being $1$.}. In this model the latent variables are $\bz = {\mathbf{\mu}, \mathbf{c}}$. From these definitions, the joint distribution factorizes as
