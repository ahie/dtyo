...
The aforementioned adaptive gradient methods aim to improve the speed of convergence. A problem that has plagued neural network training is the \textit{vanishing gradient problem}, but through the use of better network initialization methods \cite{xavier_initialization}, the use of non-saturating non-linearities, such as \cite{relu}, and special network structures, such as ResNet \cite{resnet}, the vanishing gradient problem has been mitigated in practice.

\note{regularization, \cite{dropout, batch_normalization}}

As an active area of research, questions still remain in even the very basic \tred{...}. For instance, how the choice of batch size and learning rate affect the results of the optimization \cite{large_batch_size_generalization, learning_rate_vs_batch_size}, how adaptive gradient methods are potentially detrimental to generalization in certain settings \cite{marginal_value_of_adaptive_gradient_methods}, and how little theoretical insight there is into even established methods like batch normalization \cite{how_batch_norm_works}.
