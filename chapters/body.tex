...
In literature, problems arising from high dimensionality are collectively referred to as the \textit{curse of dimensionality}, a term first coined by Richard Bellman in the context of dynamic programming \cite{bellman_curse_of_dimensionality}. Particularly in machine learning the following aspects of high dimensionality 
...
\note{short history of methods developed, RBM/DBN/DBM/GAN...}
...
\begin{figure}[!htb]
  \centering
  \resizebox{0.8\textwidth}{!}{\input{tikz/nn.tikz}\unskip}
...
A simplistic view of neural networks would be to consider them merely as powerful function approximators. The theoretical underpinning for this view is the universal approximation theorem \cite{universal_approximation_theorem}, which states that under mild conditions on the activation function, neural networks with a single hidden layer can arbitrarily well approximate any continuous function.\footnote{The universal approximation theorem requires that the activation function is nonconstant, bounded and continuous, which highlights the importance of non-linear activation functions.}$^{,}$\footnote{To be precise, this approximation applies to continuous functions defined on compact subsets of $\mathbb{R}^n$.} However, the universal approximation theorem does not state anything about the challenges associated with the learnability of the parameters for these approximations, which is where many of the practical challenges associated with neural networks lie.
...
\note{Backprop algorithm in more detail, SGD, gradient descent optimization methods (mention second order methods?)}

Several difficulties still arise in the optimization of neural networks. \note{explain some of the recent research regarding convergence, vanishing gradients, momentum based methods, regularization, training deeper networks ... \cite{batch_normalization, dropout, relu, resnet}, and how there still isn't consensus on why some established methods seem to work (\cite{how_batch_norm_works}) (learning rate/batch size choice for optimization \cite{large_batch_size_generalization, learning_rate_vs_batch_size}) (potentially worse generalization of adaptive gradient methods, such as AdaGrad, RMSProp, Adam \cite{marginal_value_of_adaptive_gradient_methods})}
...
Connection with pca demonstrated in \cite{autoencoder_pca} when activation functions are linear.

\subsection{Variational Inference}
\label{subsection:variational_inference}

In modern statistics and machine learning a need to approximate highly complex probability distributions is a problem that often arises as
Highly complex probability densities often arise in modern statistics and machine learning. 
A core challenge that arises in modern statistics and machine learning is that of approximating 
...
\cite{semi_supervised_vae, adversarial_autoencoders}
...
\note{Methods to improve the runtime of t-SNE have been subsequently proposed: \cite{accelerated_tsne_1, accelerated_tsne_2}. Use of the fast multipole method \cite{fast_multipole} has not been explored due to no suitable expansion for the t-Distributed forces in the output space exist.}

\note{Worth a mention is the Stochastic Triplet Embedding \cite{stochastic_triplet_embedding} and Multiple Maps t-SNE \cite{multiple_maps_tsne} methods developed on top of t-SNE, developed to handle non-metric similarities present in the data.}
...
\note{TODO: go over what this method attempts to improve upon. Other methods are not as scalable, various problems with curse of dimensionality, parametric t-SNE has been previously shown to be slow to optimize and produce poorer results.}
\note{Even though the VAE addresses issues with scalability and reliance on pairwise distances in the input space, the resulting latent code does not allow for as good separation of clusters as other methods, such as t-SNE would, due to the strong effect the imposed prior has on the global shape of the embedding of a data set.}

The method proposed in this work involves combining the VAE model with Parametric t-SNE to produce a general framework for improved parametric dimensionality reduction for data visualization. In this chapter the basic learning algorithm as well as a few of its variations are described.

\section{Learning a Parametric Embedding Using VAE Sampling}

\note{The aim of the method is to take advantage of the strengths of both VAE and PTSNE.}
\note{Pros of VAE: generative model, stochasticity brings robustness, scalable parametric nonlinear dimensionality reduction}
\note{Pros of PTSNE: theoretically capable of learning as good projections as t-SNE, which is widely considered SOTA. Also scalable and parametric, if trainable with smaller batch sizes.}
\note{Using the notation of the previous chapter, we can summarize the learning procedure as Algorithm~\ref{alg:learning}.}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \While{VAE not converged}
      ...
      \State $\mathbf{z}^{(i)} \sim q_{\mathbf{\phi}}(\mathbf{z} \vert \mathbf{x}^{(i)})$
    \EndWhile
    \While{PTSNE not converged}
      \State $\mathbf{X} \gets \text{Random minibatch}$
      \State $\hat{\mathbf{x}}^{(i)}$ 
      \State $\bW = ...$
    \EndWhile
  \end{algorithmic}
\end{algorithm}

\note{After training, a deterministic projection for all subsequent points can be obtained with Algorithm~\ref{alg:projection}.}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \State $\hat{\mathbf{x}} \gets \text{Deterministic sample of } \mathbf{x}$
...
From the underlying generative model we can approximately compute the marginal $p(\mathbf{x})$ for each data point $\mathbf{x}$. An approximation can be retrieved by importance sampling \cite{stochastic_backpropagation}:
...
\textcolor{red}{[Note: discuss the more accurate but computationally costly marginal estimation method: annealed importance sampling \cite{ais, ais_vae}?]}
...
To validate the proposed method properties of the combined model were studied on the five different data sets described below.
...
As real-world data is generally not perfectly clean, an important property for any machine learning method is its capability to handle data containing artifacts. Considering VAEs are inherently robust to corrupted data due to the regularization provided by the stochastic latent code (\textcolor{red}{[should be discussed in a previous section more thoroughly, connection with VAE and Robust PCA \cite{rpca_1, rpca_2} shown in \citep{hidden_talents_of_the_vae}]}) it is reasonable to hypothesize that a training procedure for a low dimensional mapping taking advantage of this property would perform better on corrupted data.
...
\todo{find citations for scrna-seq/dropout}
...
\section{Application to scRNA-seq Data}

Using the data set of \cite{tirosh} we train our model.

Due to the very high dimensionality in comparison to the other data sets, more capacity to the VAE network was added, i.e. we chose to add more units to the hidden layers. 

In Figure~\ref{fig:tirosh} we have visualized the results of our analysis as obtained by replicating parts of the original study.

Different tumor samples and cell types cluster well in the embedding. The separation of malignant cells from different patients to separate clusters reveals the inherent heterogeneity of melanoma. \note{A few separate clusters of T-cells are also present, which are not visible in the embedding of the original paper. (Add this only if using the projection where this is more clear)}

MITF and AXL program cell scores. Areas of MITF-high and AXL-high are present in the cluster of tumor clusters.

Cell cycle analysis reveals the presence of subclusters of cycling cells. Cycling cells are those that are undergoing cell division. The color gradient in the projection for cycling cells corresponds to how strong the signal indicating cell cycling is. Below a given threshold value for this signal, cells are classified as non-cycling.

The effect of the curse of dimensionality can be seen on methods based on distances. Applying UMAP directly to the high dimensional data results in for example T- and B-cell clusters merging. \note{FIXME: Add the UMAP figure.}

\begin{figure}[!htb]
  \includegraphics[width=\textwidth]{images/tirosh_combined.png}
  \caption{Compilation of analysis results replicated on data from \cite{tirosh} visualized as a 2-D scatter plot using our method. \note{FIXME: the actual figure.}}
  \label{fig:tirosh}
\end{figure}
