...
The structure of autoencoders can be decomposed into two parts, namely the encoder and decoder. As the constituents' names suggest, part of the network performs encoding of the input and the other decoding of the encoded input, hence the name of the network as a whole refers to "self-encoding". An example autoencoder network has been visualized in Figure~\ref{fig:ae}.

\tred{Through such encoding the network attempts to learn to produce an efficient code capable of capturing salient features of the data it has been trained to autoencode.}

\tred{Trivial case, where the network has too much capacity the AE can simply learn an identity mapping. This is clearly not conducive to learning meaningful latent representations of the data.}
...
The learning objective of autoencoders is for the network to be able to reconstruct its inputs as faithfully as possible. A common objective function to train an autoencoder with is to have it minimize the mean squared error between its input and output:
