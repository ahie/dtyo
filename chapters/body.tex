...
An essential component of the method presented in this work are Variational Autoencoders (VAEs). VAEs belong to a broad class of models called deep generative models, which have in recent years played a pivotal role in machine learning research. As their name suggests, generative models attempt to model the process by which a given set of data has been generated.
...
In Bayesian inference one of the core challenges is the difficulty of computing the posterior of statistical models. Even for only moderately complex statistical models exact computation of the posterior is often intractable and an approximation is sought instead. The dominant paradigm for obtaining these approximates has been through various methods of sampling.

The Bayes theorem states

$$p(\bz \vert \bx) = \frac{p(\bz, \bx)}{p(\bx)} = \frac{p(\bz, \bx)}{\int p(\bz, \bx) d\bz}.$$

Bayesian mixture of Gaussians as example of simple hierarchical model with intractable marginalization:
