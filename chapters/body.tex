...
To formalize these notions, we begin with how the probability distribution in the input space is defined. 

$$p_{ij} = \frac{exp(-\norm{\mathbf{x}_i - \mathbf{x}_j}^2 / 2 \sigma_i^2)}{\sum_{k \neq i} exp(-\norm{\mathbf{x}_i - \mathbf{x}_k}^2 / 2 \sigma_i^2)}$$

where $\sigma_i$ is determined separately for each data point $i$ by forcing the perplexity of $P_i$ equal to some preset value. Perplexity is defined as

$$2^{H(p(\mathbf{x}))} = 2^{-\sum_{i=1}^{N} p(\mathbf{x}^{(i)}) \log_2 p(\mathbf{x}^{(i)})}.$$

From the definition we can see that a larger $\sigma$ will be chosen in low density areas and a smaller $\sigma$ will be used in high density areas. Effectively, $\sigma$ is adapted to fit the core assumption that the data points lie on a uniformly sampled manifold within the high dimensional ambient manifold $\mathbb{R}^d$. Perplexity is thus a value proportional to the number of effective local neighbors considered by the algorithm when the embedding is being optimized.

The joint distribution in the output space is similarly defined with a Gaussian kernel:

$$q_{ij} = \frac{exp(-\norm{\mathbf{y}_i - \mathbf{y}_j}^2)}{\sum_{k \neq i} exp(-\norm{\mathbf{y}_i - \mathbf{y}_k}^2)},$$

where each $\mathbf{y}_i$ represents the point in the embedding for data point $i$. A notable difference is the absence of the variance term $2 \sigma_i^2$, which can be interpreted as having been set to $1$ throughout.

Now that we have defined both the output and input distributions for points, the cost function can be formulated as the sum of KL-divergences between each.

$$C = \sum_i \dkl{P_i}{Q_i} = \sum_i \sum_j p_{ij} \log \frac{p_{ij}}{q_{ij}}$$

An important fact to note is that the cost function is in fact non-convex, which implies the unfortunate property of the cost function having multiple local minima, i.e. the optimization of the function is not guaranteed to terminate at the global optimum. With random restarts and other optimization methods, such as stochastic gradient descent and gradient descent with momentum, it is possible to attempt to escape poor local minima.
...
$$\frac{\partial C}{\partial \mathbf{y}_i} = 2 \sum_j (\mathbf{y}_i - \mathbf{y}_j)(p_{ij} - q_{ij} + p_{ji} - q_{ij})$$

This has the intuitive interpretation of a force directed layouting algorithm, where each pair of points is either pushing or pulling on each other, depending on whether they are less or more likely to be neighbors under the output's joint distribution than the input's.
...
One of the main issues t-SNE aims to address in SNE is the so called \textit{crowding problem}. The crowding problem arises as a consequence of ....
