...
\item $\dkl{p}{q} = 0 \iff p = q$ (identity of indeiscernibles).
\end{enumerate}
...
where the probability can be seen as being derived as if a Gaussian was centered on $\mathbf{x}_i$ and an appropriate normalizing term is used to scale the individual probabilities so that they sum to $1$. The variance (denoted by $\sigma_i$ in the equation) is determined separately for each data point $i$ by forcing the perplexity of the conditional distribution over all other data points (later denoted as $P_i$) to equal some preset value.
