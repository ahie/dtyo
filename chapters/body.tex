...
\note{This chapter should be split into the following sections:}
...
The fundamental quantity of interest in information theory, entropy, is the average amount of information produced by a stochastic source. \tred{In more concrete terms ...} Entropy computes the expected value of the log-probability of a random variable. A coin flip carries only one bit of information, whereas a roll of a 6-sided die carries approximately $2.58$ bits of information.
...
\begin{equation}
q^{*}(\mathbf{z}) = \argmin_{q(\mathbf{z}) \in \mathcal{Q}} \dkl{q(\mathbf{z})}{p(\mathbf{z} \vert \bx)},
\label{eq:vi}
\end{equation}

\note{where the quantity being optimized is the KL-divergence introduced in the previous section. The optimization tries to find a member $q^{*}(\mathbf{z})$ of the chosen family of distributions that best matches the true posterior, which can then be used as a surrogate for the true posterior.}
...
\begin{equation}
\begin{aligned}
\dkl{q(\mathbf{z})}{p(\mathbf{z} \vert \bx)} &= \mathbb{E}[\log q(\mathbf{z})] - \mathbb{E}[\log p(\mathbf{z} \vert \bx)] \\
                                             &= \mathbb{E}[\log q(\mathbf{z})] - \mathbb{E}[\log p(\mathbf{z}, \bx)] + \log p(\bx),
\end{aligned}
\end{equation}
...
\begin{equation}
\begin{aligned}
\log p(\bx) &= \dkl{q(\mathbf{z})}{p(\mathbf{z} \vert \bx)} - \mathbb{E}[\log q(\mathbf{z})] + \mathbb{E}[\log p(\mathbf{z}, \bx)] \\
            &= \underbrace{\dkl{q(\mathbf{z})}{p(\mathbf{z} \vert \bx)}}_{\geq 0} + \underbrace{\mathbb{E}[\log p(\bx \vert \mathbf{z})] - \dkl{q(\mathbf{z})}{p(\mathbf{z})}}_{\text{ELBO}}
\end{aligned}
\end{equation}
...
$$\mathbb{E}_{\qphi(\bz \vert \bxi)}[f(\mathbf{z})] \simeq \frac{1}{L} \sum_{l=1}^{L} f(g_{\bphi}(\bxi, \eil))$$
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \While{VAE not converged}
      ...
      \ForAll{$\bxi \in \mathbf{X}$}
        \For{$l \gets 1$ to $L$}
          ...
          \State $\hat{\bx}^{(i,l)} \gets f_{\mathbf{\theta}}(\bzil)$ \Comment{Obtain parameters of $\ptheta(\bx \vert \bzil)$ by decoding $\bzil$}
        \EndFor
    ...
    \While{PTSNE not converged}
      ...
      \EndFor
      \State $\bW \gets \text{ Update parameters by descending gradient } \nabla_{\bW} C_{\text{t-SNE}} \text{ using all computed } \widetilde{\bx}^{(i)} \text{ and } \mathbf{y}^{(i)}$
    \EndWhile
  \end{algorithmic}
\end{algorithm}
...
$$p(\bx) \simeq \frac{1}{N} \sum_{i=1}^{N} \frac{p(\bx \vert \mathbf{z}^{(i)}) p(\mathbf{z}^{(i)})}{q_{\theta}(\mathbf{z}^{(i)} \vert \bx)},$$

where $\mathbf{z}^{(i)} \sim q_{\theta}(\mathbf{z} \vert \bx)$. Alternatively, one could directly use the ELBO to estimate $p(\bx)$; however, this gives generally worse estimates \cite{ais_vae}. For more accurate estimates than importance sampling, the usage of annealed importance sampling \cite{ais} has been suggested for VAEs in \cite{ais_vae}.
