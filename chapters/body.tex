...
The Stochastic Neighbor Embedding (SNE) algorithm \cite{sne}, developed by Hinton and Roweis in 2003, serves as the precursor to the later improved t-SNE algorithm. As such, SNE shares many similarities in its construction with t-SNE, and thus by first developing an understanding of SNE and its pitfalls the motivation behind the t-SNE algorithm become more apparent.

As the algorithm's name suggests, the approach SNE takes to perform dimensionality reduction is probabilistic in nature. Without delving directly into the mathematical details, an intuitive overview of the algorithm is that it assigns a probability to the event that each pair of points gets picked in the input space, after which the distribution of points in the output space is attempted to be matched with that of the input space under a similar probability measure. The natural way to match two distributions is familiar from the discussion on variational bayes methods, namely the KL-divergence between these two is used as a loss function for the embedding of data points, which can be optimized with various methods, including the previously covered gradient descent.

To formalize these notions, we begin with how the conditional probability distribution $p_{j \vert i}$ in the input space is defined by the equation

$$p_{j \vert i} = \frac{exp(-\norm{\mathbf{x}_i - \mathbf{x}_j}^2 / 2 \sigma_i^2)}{\sum_{k \neq i} exp(-\norm{\mathbf{x}_i - \mathbf{x}_k}^2 / 2 \sigma_i^2)},$$

where $\sigma_i$ is determined separately for each data point $i$ by forcing the perplexity of $P_i$ equal to some preset value. Perplexity is an information theoretic measure of how well a distribution predicts its samples. For a given probability distribution, perplexity is defined as the exponentiation of the distribution's Shannon entropy, e.g. in the case of our discrete $P_i$:

$$b^{H(P_i)} = b^{-\sum_{j=1}^{n} p_{j \vert i} \log_b p_{j \vert i}},$$

where $b$ is the unit used to measure entropy in, typically chosen to be either $2$ or $e$, for bits or nats, respectively. From the definition we can see that larger $\sigma$ will be chosen in low density areas and smaller $\sigma$ will be used in high density areas. Effectively, $\sigma$ is adapted to fit the core assumption that the data points lie on a uniformly sampled manifold within the high dimensional ambient manifold $\mathbb{R}^d$. Perplexity is thus a value proportional to the number of effective local neighbors considered by the algorithm when the embedding is being optimized.
...
where each $\mathbf{y}_i$ represents the point in the embedding for data point $\mathbf{x}_i$. A notable difference is the absence of the variance term $2 \sigma_i^2$, which can be interpreted as having been set to $1$ throughout.
...
\begin{equation}
C = \sum_i \dkl{P_i}{Q_i} = \sum_i \sum_j p_{j \vert i} \log \frac{p_{j \vert i}}{q_{j \vert i}}.
\label{eq:c_sne}
\end{equation}
...
Gradient descent is typically used to optimize the embedding. The partial derivatives w.r.t. each point in the embedding is given by

\begin{equation}
\frac{\partial C}{\partial \mathbf{y}_i} = 2 \sum_j (\mathbf{y}_i - \mathbf{y}_j)(p_{j \vert i} - q_{j \vert i} + p_{i \vert j} - q_{i \vert j})
\label{eq:c_sne_grad}
\end{equation}
...
The t-SNE algorithm aims to address two issues of the original SNE algorithm. First, through considering joint distributions, instead of conditional distributions, the form of the gradient of the objective function becomes easier to compute. Second, one of the main issues with plain SNE is the so called \textit{crowding problem}, which is addressed by considering a heavier tailed probability distribution function in the output space, namely Student's t-Distribution. Together these improvements enable better separation of natural clusters in the output space of the algorithm, as compared to the original SNE method.

Symmetric SNE \cite{symmetric_sne} aims to provide an objective function whose gradient is easier to compute than the one of Equation~\ref{eq:c_sne_grad}. 
...
$$C_{\text{t-SNE}} = \sum_i \dkl{P_i}{Q_i} = \sum_i \sum_j p_{ij} \log \frac{p_{ij}}{q_{ij}}.$$

For which the following gradient can be derived, w.r.t. each $\mathbf{y}_i$:

$$\frac{\partial C_{\text{t-SNE}}}{\partial \mathbf{y}_i} = $$

To alleviate the crowding problem, 

Methods to improve the runtime of t-SNE have been subsequently proposed: \cite{accelerated_tsne_1, accelerated_tsne_2}. The use of the fast multipole method \cite{fast_multipole} has not been explored due to no suitable expansion for the t-Distributed forces in the output space exist.
...
t-SNE does not directly enable out of sample extension, but random-walk t-SNE has been developed for this purpose. Although the following pitfalls are present in this method: ...
