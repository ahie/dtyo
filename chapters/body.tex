...
\note{Elaborate the potential downsides of this approach (how to interpret the learned representation when nonlinearity is introduced? how does it generalize?). This is set up for why stochastic autoencoders are desireable. "Even though autoencoders provide a powerful and scalable non-metric approach to dimensionality reduction there are a few disadvantages inherent to how they function. ..."}
...
\note{Even though the VAE addresses issues with scalability and reliance on pairwise distances in the input space, the resulting latent space representation does not allow for as good separation of clusters as other methods, such as parametric t-SNE or UMAP \cite{umap}, which will later be demonstrated in Section~\ref{section:comparisons}.}
...
The aim of the method is to take advantage of the strengths of both VAE and PTSNE.
...
Several experiments are carried out to validate the efficacy of the proposed method, as well as compare it to existing methods. \note{Add brief explanation of what the experiments cover.} Lastly, two applications of the method to single cell data analysis are presented in section \ref{section:applications}.
