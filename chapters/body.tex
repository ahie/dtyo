...
First discussed in \cite{encoding_problem} as "the encoding problem", autoencoders are a class of neural network architectures designed for unsupervised learning.
...
The learning objective of autoencoders is for the network to be able to reconstruct its inputs as faithfully as possible. 

A common objective function to train an autoencoder with is to have it minimize the mean squared error:
...
\note{This chapter should cover at least entropy, relative entropy (KL-div).}

The Kullback-Leibler divergence, or KL-divergence for short, is a measure of dissimilarity between two probability distributions. \note{KL-divergence aka relative entropy to tie the two together}
...
In modern statistics and machine learning a need to approximate highly complex probability distributions is a problem that often arises. A domain where such complex distributions are often encountered is Bayesian inference. The Bayes theorem states that the posterior distribution is given by

$$p(\bz \vert \bx) = \frac{p(\bz, \bx)}{p(\bx)} = \frac{p(\bz, \bx)}{\int p(\bz, \bx) d\bz},$$

\note{where ... explain the variables ...} Even for only moderately complex models exact computation of the posterior is often intractable due to the required marginalization over all possible values of $\mathbf{z}$.
...
\note{where the quantity being optimized is the KL-divergence introduced in the previous chapter. The optimization tries to find a member $q^{*}(\mathbf{z})$ of the chosen family of distributions that best matches the true posterior, which can then be used as a surrogate for the true posterior.}
...
and by rearranging, it follows that
...
VAEs extend autoencoders into probabilistic generative models. Instead of the encoding and decoding networks of traditional autoencoders, VAE encoders and decoders produce conditional distributions of data and their corresponding latent variables. Figure~\ref{fig:aevb_plate_notation} depicts the graphical model under consideration, where $\bphi$ and $\btheta$ are the parameters of the encoder and decoder, respectively.
...
It is important to note that the output $\hat{\mathbf{x}}^{(i,l)}$ of the network in Figure~\ref{fig:vae} should not be interpreted as the final reconstruction of the input like in traditional autoencoders, but rather as the values parameterizing the distribution $\ptheta(\mathbf{x} \vert \bz^{(i, l)})$.
...
\begin{figure}[!htb]
  \centering
  \resizebox{0.8\textwidth}{!}{\input{tikz/vptsne.tikz}\unskip}
...
\note{As we can draw infinite samples from the continuous distribution $p_{\bphi}(\bz \vert \bxi)$ with $g_{\bphi}(\eil, \bxi)$ ...}
