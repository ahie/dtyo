...
\item from a more practical perspective, high dimensional data by itself is hard to directly reason about and difficult to visualize in any meaningful sense. Simple statistics will often not capture 
\end{itemize}

The given listing is not exhaustive, but to serve as an illustration for the variety of issues that can be faced.
...
Neural networks can be thought of as being large function compositions. Each neuron that is part of the network acts as a single function, taking as input the weighted output of other neurons connected to it and outputting the value of the neuron's \textit{activation function}. Neural networks can be classified into two main categories: feedforward and recurrent neural networks. Thinking of neurons and their connections in terms of graphs, these categories correspond to acyclic directed graphs and directed graphs that may contain cycles, respectively. 

The purpose of an activation function is to alter incoming signals to induce non-linearity into the network as well as impose constraints on the values that flow through specific neurons, enabling the learning of more complex functions with desired properties. 

A common representation of a simple neural network is a graph where the output-input neuron pairs are visualized with directed edges. As an example, Figure~\ref{fig:nn} depicts a feedforward neural network\footnote{Note that bias nodes are omitted from the figure for simplicity.} taking input vectors $\mathbf{x}$ of length $3$ and outputting vectors $\mathbf{y}$ of length $2$. The output of the depicted neural network can be expressed with the equation

$$\mathbf{y} = f_3(\mathbf{W}_3 f_2(\mathbf{W}_2 f_1(\mathbf{W}_1 \mathbf{x} + \mathbf{b}_1) + \mathbf{b}_2) + \mathbf{b}_3),$$

where $f_i$ are the activation functions, $\mathbf{W}_i$ the weights, and $\mathbf{b}_i$ the biases of the $i$th layer.
