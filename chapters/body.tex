...
\item find references where visualization is used to aid analysis
...
The learning objective of autoencoders is to reconstruct the original input data as faithfully as possible. \note{Common objective function is to minimize the mean squared error:}
...
\note{Structure consists of two parts, the encoder and decoder. An example autoencoder network has been visualized in Figure~\ref{fig:ae}.}

\note{Trivial case, where the network has too much capacity the AE can simply learn the identity function. Not useful for learning hidden representations, i.e. meaningful latent codes.}

\note{Case where the latent code's dimensionality is smaller than the original data's dimensionality. This \textit{bottleneck} potentially discards data, leading to a \textit{lossy compressed} representation.}
...
A connection between PCA and autoencoders which are constrained to only have linear activation functions and are optimized with the MSE objective (see Equation~\ref{eq:mse}) has been shown in \cite{autoencoder_pca}. \note{The subspace spanned by the first $d'$ principal eigenvectors of the (combined) weight matrix is equivalent to the first $d'$ principal components of the training data. The latent code of such an autoencoder is an orthogonal projection onto this subspace. Additionally, the optimization problem is convex.}

\note{Elaborate potential downsides of this approach (how to interpret the learned representation when nonlinearity is introduced? how does it generalize?). This is set up for why stochastic autoencoders are desireable.}
...
$$\mathbf{z}^{(i,l)} = g_{\bphi}(\eil, \bxi), \eil \sim p(\mathbf{\epsilon})$$
...
\note{Methods to improve the runtime of t-SNE have been subsequently proposed: \cite{accelerated_tsne_1, accelerated_tsne_2}. Use of the fast multipole method \cite{fast_multipole} has not been explored due to no suitable multipole expansion for the t-Distributed forces in the output space exist.}

\note{Worth a mention are the Stochastic Triplet Embedding \cite{stochastic_triplet_embedding} and Multiple Maps t-SNE \cite{multiple_maps_tsne} methods developed on top of t-SNE, developed to handle non-metric similarities present in the data.}

\subsection{Parametric t-SNE}
\label{subsection:parametric_tsne}

A major drawback of most nonlinear dimensionality reduction methods is that they do not directly enable out-of-sample extensions.
...
The method proposed in this work involves combining the VAE model with Parametric t-SNE to produce a general framework for improved parametric dimensionality reduction for data visualization. In this chapter the learning algorithm as well as a variation of it is described.
  ...
  \begin{algorithmic}[1]
    ...
    \While{VAE not converged}
      ...
      \ForAll $\bxi \in \mathbf{X}$
        \State $\eil \gets$ 
        \State $\bz^{(i,l)} \gets g_{\phi}(\eil, \bxi)$
    ...
    \While{PTSNE not converged}
      ...
      \State $\bW \gets \text{ Update parameters with } \bxi$
    \EndWhile
  \end{algorithmic}
\end{algorithm}
...
\note{For encoder networks that directly parameterize the mean of a distribution, computing the expectation can be done exactly with zero overhead.}
...
\begin{itemize}
\item We are performing a step of nonlinear dimensionality reduction that is not dependent on local distance metrics, i.e. a preprocessing step that is less susceptible to the curse of dimensionality.
...
\note{Experiments are carried out to validate the efficacy of the method, as well as compare it to previous methods.} Lastly, two applications of the method to single cell data analysis are presented in sections \ref{section:inference_with_the_generative_model} and \ref{section:application_to_scrna}.
...
To empirically validate the claims of advantageous properties of the proposed method the combined model was studied on the five different data sets described below.
